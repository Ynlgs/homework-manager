...
class Homework(WorkAbstractModel):
    ...
    def get_absolute_url(self):
        return reverse('project:homework_detail',
                        args=[str(self.course.id), str(self.pk)])
...
class Handin(WorkAbstractModel):
    ...
    # Django 的 CreateView 和 UpdateView 在完成对象的创建或编辑后会自动跳转到这个绝对 url
    def get_absolute_url(self):
        return reverse('project:handin_detail',
                        args=[
                            str(self.homework.course.id),
                            str(self.homework.pk),
                            str(self.pk)
                        ])
