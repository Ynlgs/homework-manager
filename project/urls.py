...
urlpatterns = [
    ...
    re_path(r'^course/(?P<pk>\d+)/update/$',
            views.CourseUpdate.as_view(),
            name='course_update'),       # 教师更新课程
    re_path(r'^course/(?P<pk>\d+)/delete$',
            views.CourseDelete.as_view(),
            name='course_delete'),       # 教师删除课程
    re_path(r'^course/(?P<pk>\d+)/select$',
            views.course_select,
            name='course_select'),  # 学生加入课程
    re_path(r'^course/(?P<pk>\d+)/cancel$',
            views.course_cancel,
            name='course_cancel'),  # 学生退出课程
    # re_path(r'^course/(?P<pk>\d+)/list$',
    #         views.HomeworkList,
    #         name='homework_list'),              # 学生进入课程 记得注释！！！
    # re_path(r'^course/(?P<pk>\d+)/homework/publish/$', views.HomeworkListPublished, name='homework_list_published'),  # 教师进入课程 记得注释！！！
    # 教师发布作业 增 删 查 改
    re_path(r'^course/(?P<pk>\d+)/homework/create/$',
            views.HomeworkCreate.as_view(),
            name='homework_create'),            # 创建作业
    re_path(r'^course/(?P<pkr>\d+)/homework/(?P<pk>\d+)/$',
            views.HomeworkDetail.as_view(),
            name='homework_detail'),            # 作业详情
    re_path(r'^course/(?P<pkr>\d+)/homework/(?P<pk>\d+)/update/$',
            views.HomeworkUpdate.as_view(),
            name='homework_update'),            # 更新作业
    re_path(r'^course/(?P<pkr>\d+)/homework/(?P<pk>\d+)/delete$',
            views.HomeworkDelete.as_view(),
            name='homework_delete'),            # 删除作业
    re_path(r'^course/(?P<pk>\d+)/homework/draft/$',
            views.HomeworkListDraft.as_view(),
            name='homework_list_publishing'),   # 待发布作业列表
    re_path(r'^course/(?P<pk>\d+)/homework/publish/$',
            views.HomeworkListPublished.as_view(),
            name='homework_list_published'),    # 已发布作业列表
    re_path(r'^course/(?P<pkr>\d+)/homework/(?P<pk>\d+)/publish/$',
    ...
    re_path(r'^course/(?P<pk>\d+)/handin/list$',
            views.HandinListDone.as_view(),
            name='handin_list_done'),       # 已提交的作业列表
    re_path(r'^course/(?P<pkr>\d+)/homework/(?P<pk>\d+)/handin/create/$',
            views.HandinCreate.as_view(),
            name='handin_create'),          # 创建提交的作业
    re_path(r'^course/(?P<pka>\d+)/homework/(?P<pkr>\d+)/handin/(?P<pk>\d+)/$',
            views.HandinDetail.as_view(),
            name='handin_detail'),          # 提交的作业详情
    re_path(r'^course/(?P<pka>\d+)/homework/(?P<pkr>\d+)/handin/(?P<pk>\d+)/update/$',
            views.HandinUpdate.as_view(),
            name='handin_update'),          # 更新提交的作业
    re_path(r'^course/(?P<pka>\d+)/homework/(?P<pkr>\d+)/handin/(?P<pk>\d+)/delete/$',
            views.HandinDelete.as_view(),
            name='handin_delete'),          # 删除提交的作业

    # 学生作业统计
    re_path(r'^course/(?P<pkr>\d+)/homework/(?P<pk>\d+)/count$',
            views.HomeworkHandin.as_view(),
            name='homework_handin_count'),
    # 学生统计
    re_path(r'^course/(?P<pk>\d+)/student$',
            views.course_student,
            name='course_student'),
    # 评论功能
    re_path(r'^comment/(?P<pk>[0-9]+)/$',
